...
| R2-6 | **Autotune**: persist `_build_metrics`/`_build_run_stats` to `~/.local/share/natshell/metrics/`, feed back into `n_ctx`/`max_tokens` selection (`scaling.py`, `_infer_context_size`). | R2§7 | 🚧 RECORDING DONE / FEEDBACK PENDING | **Recording half shipped (2026-09-13):** new `agent/run_metrics.py` — append-only JSONL in `data_dir()/metrics/natshell.jsonl`, `0o700` dir, module-wide write lock, temp+`os.replace` truncation at 1000 lines, per-line `schema:1`, `load_recent(n)` (skips malformed lines), `stats()` summary, singleton + `reset_run_metrics_store()` for tests (mirrors `backup.py`/`session.py` shape). `NATSHELL_DISABLE_METRICS` kills the default-location store (set in `tests/conftest.py` so the suite never writes into the real data dir; explicit `dir_path` stores unaffected). Loop: `handle_user_message`'s main body wrapped in `try/finally` — `finally` calls new `_record_run(stats, steps_used)`, best-effort (store never raises; engine `engine_info()` failures degrading to "no metadata" don't affect recording), so every terminal path (response, error, max-steps, degenerate, truncated, escaping exception) appends exactly one record with engine_type/model/n_ctx attached when the engine reports them. 21 new tests in `tests/test_r26_run_metrics.py` (bootstrap/perms, append/load round-trip, truncation, malformed-line tolerance, disable + kill switch, 4 AgentLoop integration tests incl. "broken store never breaks the run"); suite **1802 green**. **Feedback half (next ⏳)**: consume `load_recent()` to inform `scaling.py` / `_infer_context_size` — deliberately separate so recording is in production first. |
