...
| R2-6 | **Autotune**: persist `_build_metrics`/`_build_run_stats` to `~/.local/share/natshell/metrics/`, feed back into `n_ctx`/`max_tokens` selection (`scaling.py`, `_infer_context_size`). | R2§7 | ✅ DONE (max_tokens feedback) / ⏳ n_ctx inference | **Recording half shipped (2026-09-13):** new `agent/run_metrics.py` — append-only JSONL in `data_dir()/metrics/natshell.jsonl`, `0o700` dir, module-wide write lock, temp+`os.replace` truncation at 1000 lines, per-line `schema:1`, `load_recent(n)` (skips malformed lines), `stats()` summary, singleton + `reset_run_metrics_store()` for tests (mirrors `backup.py`/`session.py` shape). `NATSHELL_DISABLE_METRICS` kills the default-location store (set in `tests/conftest.py`); loop records exactly one row per run on every terminal path (best-effort, `try/finally`), engine_type/model/n_ctx attached when reported. **Feedback half shipped (2026-09-13, this commit):** `RunStats.saw_truncation` set at the `finish_reason=="length"` site, stamped into the persisted record; `scaling.advise_max_tokens` (pure, 3 independent growth gates: ×max_multiplier, +min_increase, ≤max_value; window + min_truncated lookback gates); `AutotuneConfig` `[autotune]` section (default `max_tokens = false` → zero behaviour change) wired from `__main__.py` → `AgentLoop._setup_context_manager` (best-effort: raising store / no history just keeps the scaled value). 34 new tests (unit policy + loop wiring; incl. "broken store never breaks setup"). Suite **1825 green** (was 1781). **Remaining (⏳):** the `n_ctx` inference half — choosing a context window for a *downloaded* model from file size (plan mentions `_infer_context_size`); not blocked by anything here but a separate unit (needs `local.py` model loading + disk-size heuristics + tests), deliberately not mixed in. |
...
- **2026-09-13** R2-6 FEEDBACK HALF SHIPPED (max_tokens autotune) —
  ...
  The remaining R2-6 item (auto-inferring `n_ctx` from a downloaded
  model's size) is a separate unit, not blocked, and deliberately not
  mixed in.
