...
| R2-4 | `execute_shell`: **stream** stdout to the TUI as it arrives + **background** handle/`tail`. | R2§6 | ⏳ STREAMING DONE · BG PENDING | **Streaming half shipped (`8348e46` + `6d4c03d`, 2026-09-10):** `stream_execute_shell` (asyncio subprocess, 4 KiB stdout pump, concurrent stderr drain, `asyncio.timeout` → kill + exit-124 parity, missing-shell 127 parity) reuses the *same* `_effective_timeout` / `_filtered_env` / `_prepare_sudo` / `_scrub_sudo_prompt` helpers the blocking path now uses (refactored onto them, no behaviour change). `ToolRegistry.register_streaming`/`execute_streaming` own "which tools stream"; `dispatch_tool_call/batch` gained a `stream_output` switch (default **False** ⇒ every existing caller, headless, plans and the mocked `tools.execute` tests keep the historical path) that surfaces each chunk as `EventType.TOOL_OUTPUT` in arrival order between EXECUTING and TOOL_RESULT. TUI: `run_agent`/`run_plan` opt in; `app.py` routes TOOL_OUTPUT → `CommandBlock.set_partial` (new `set_partial`/final-`set_result` pair, copyable mid-stream), verified end-to-end via a Textual pilot. Headless: TOOL_OUTPUT is a documented no-op. 16 new tests (`tests/test_stream_execute_shell.py`): chunk order, byte-parity (truncation + sudo scrub), timeout-124 shape, sudo stdin transport + pkg `y\n`, 127, loop event ordering + default-off, TUI partial→final. Suite **1689 green** (was 1673). **Remaining sub-step (next tick, API decided below):** background handle + `tail`. Chosen API: a small `shell_bg` tool module (`src/natshell/tools/shell_bg.py`) with three SAFE-ish/CONFIRM actions over a `~/.local/share/natshell/bg/` (0o700, `data_dir()`-based) handle dir: `launch(command)` → detached `start_new_session` child, stdout+stderr appended to `<id>.log`, handle JSON in the dir (id = 16-hex, pid, started_at, finished, exit_code); `tail(handle, lines=50)` → status + log tail; `kill(handle)` → `killpg` SIGTERM→SIGKILL after 2 s. Lifetime: session-scoped (in-memory proc registry + pid-based kill after restart; orphan logs cleaned up at startup for handles >24 h). All three classify CONFIRM by the fails-closed default (no classifier rule needed); launch's `command` arg is classified exactly like `execute_shell`'s. Prompt overhead concern: keep the three actions as one `shell_bg(action, ...)` tool rather than three definitions, and leave them **out** of `SMALL_CONTEXT_TOOLS` so ≤8K windows keep today's surface. Do NOT build until the streaming half has had a real TUI soak. |
