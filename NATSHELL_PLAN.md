...
| R1-6 | Extraction of recovery state machine to `recovery.py` | R1§1.3 | ⏳ PARTIAL | `agent/fallback.py` exists; but `_try_local_fallback` (loop.py:979), the overflow→fallback→restore ladder, and the `_context_recovery_attempted` flag are still inline in `handle_user_message`. Model as an ordered `RecoveryStrategy` object. |
