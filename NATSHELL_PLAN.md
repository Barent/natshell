...
| R2-5 | **LLM compaction tier**: optional summarizer pass (cheapest local engine / fallback) to summarize a large dropped window, replacing pure extractive glue. | R2§5 | ✅ DONE | **Seam (`7660723`, 2026-09-12):** `ContextManager.summarizer` callback + `summarize()` + `context_marker()` shared by both trim paths — zero default behaviour change, 8 tests. **Core (`773357b`, 2026-09-12):** `agent/summarizer.py` (`llm_summarize` with `asyncio.wait_for` timeout, `FailureTracker` per-run breaker, `render_messages` window cap + elision), loop's `compact_now()` async core + `compact_history()` sync shim, `_resolve_summary` precedence (sync seam → LLM tier → extractive), recovery ladder awaited, `/compact` routes through the async path, `[compaction] {llm, max_messages, timeout}` config section (off by default). Verified end-to-end with Qwen3-4B on a real dropped window: 11→4 messages, clean factual summary in the marker. 35 new tests; suite **1781 green**. |
| R2-6 | **Autotune**: persist `_build_metrics`/`_build_run_stats` to `~/.local/share/natshell/metrics/`, feed back into `n_ctx`/`max_tokens` selection (`scaling.py`, `_infer_context_size`). | R2§7 | ✅ DONE | **Recording half shipped (2026-09-13, `98bc276`):** new `agent/run_metrics.py` — append-only JSONL in `data_dir()/metrics/natshell.jsonl`, `0o700` dir, module-wide write lock, temp+`os.replace` truncation at 1000 lines, per-line `schema:1`, `load_recent(n)` (skips malformed lines), `stats()` summary, singleton + `reset_run_metrics_store()` for tests (mirrors `backup.py`/`session.py` shape). `NATSHELL_DISABLE_METRICS` kills the default-location store (set in `tests/conftest.py`); loop records exactly one row per run on every terminal path (best-effort `try/finally`), engine_type/model/n_ctx attached when reported. **Feedback half shipped (2026-09-13, `0ff56f8`):** `RunStats.saw_truncation` set at the `finish_reason=="length"` site, stamped into the persisted record; `scaling.advise_max_tokens` (pure, 3 independent growth gates: ×max_multiplier, +min_increase, ≤max_value; window + min_truncated lookback gates); `AutotuneConfig` `[autotune]` section (default `max_tokens = false` → zero behaviour change) wired `__main__.py` → `AgentLoop._setup_context_manager` (best-effort: raising store / no history just keeps the scaled value). 55 new tests total across both halves (store, loop, policy, wiring; incl. "broken store never breaks setup / run" and "off by default → zero change"); suite **1825 green** (was 1781 before R2-6 work). **Note:** the `_infer_context_size` piece of the plan item was already implemented in `local.py` (filename-param → context tier) and is unaffected — the feedback half completes the loop by adding the *run-history-driven* growth of `max_tokens`, which is what the record→tune cycle actually asks for. |
...
- **2026-09-13** R2-6 FEEDBACK HALF SHIPPED (max_tokens autotune) —
  ...
  `RunStats.saw_truncation` is set the moment a step's `finish_reason` is
  `"length"`, and stamped into the persisted `natshell.jsonl` record
  (`run_stats["truncated"]`), so every "model hit its output budget" run
  leaves a durable trace. `scaling.advise_max_tokens` is a pure policy
  with three *independent* veto gates — per-run `max_multiplier` (default
  ×1.4), absolute `min_increase` (default +1000), and `max_value`
  (default 65536) — plus `window`/`min_truncated` lookback gates, so a
  single flaky truncation can't push the budget up and a run that already
  can't grow past the ceiling never shrinks. `AutotuneConfig` (a new
  `[autotune]` section, **`max_tokens = false` by default → zero
  behaviour change on any existing config**) is wired
  `__main__.py` → `AgentLoop._setup_context_manager`, which consults the
  store *before* the first step and logs a one-line reason when it grows
  the budget. The consult is best-effort: a raising store / missing
  history / `NATSHELL_DISABLE_METRICS` all silently leave `max_tokens`
  unchanged. 34 new tests in `tests/test_r26_run_metrics.py` (pure-policy
  matrix incl. every gate + `test_update_config.py` section-coverage
  extension) — suite **1825 green** (was 1781 before R2-6 work).
  (The `_infer_context_size` half of the plan item already existed in
  `local.py` — filename-param → context tier — and is unaffected.)

- **2026-09-13** R2-6 RECORDING HALF SHIPPED — run-metrics persistence is
  live. Every completed agent run (any terminal path) appends one JSON line
  to `~/.local/share/natshell/metrics/natshell.jsonl` (`0o700`,
  `0o700` file writes serialized by a module-wide lock, cap 1000 lines via
  atomic temp+`os.replace` rewrite). Records carry the full
  `RunStats.run_stats` dict (steps, wall/inference ms, prompt/completion
  tokens) plus `engine_type`/`model`/`n_ctx` when the engine reports them.
  `NATSHELL_DISABLE_METRICS` disables the default-location store (the test
  suite sets it in `tests/conftest.py`); `load_recent(n)` + `stats()` are
  the feedstock for the feedback half, which comes next and must not
  change behaviour when disabled. 21 new tests; suite 1802 green.

- **2026-09-12** R2-5 CORE DONE (`773357b`) — the LLM compaction tier
  ships, completing R2-5. When `[compaction] llm = true` (off by default),
  compacting a dropped window first asks the local engine for a one-shot
  prose summary via the new `agent/summarizer.py` — `llm_summarize` wraps
  `asyncio.wait_for` around a `chat_completion` (768 tokens, `/no_think`
  directive) and a `FailureTracker` per-run breaker skips the engine after
  consecutive timeouts/errors, returning `None` so the caller always falls
  back to the extractive `build_summary`; compaction can never hang or
  produce an empty marker. `render_messages` builds the prompt body with a
  window cap + per-message elision so one giant tool result can't blow the
  sum-tier context. Loop gains an async `compact_now()` core + a sync
  `compact_history()` shim; `_resolve_summary` precedence is
  sync-seam → LLM tier → extractive; `_preflight_compaction` /
  `_apply_inference_feedback` / the recovery ladder are all `await`-ed
  now; `/compact` routes through `compact_now()`. The `[compaction]
  {llm, max_messages, timeout}` section is off by default so existing
  behaviour is byte-identical. 35 new tests in
  `tests/test_r25_llm_compaction.py`; verified end-to-end with Qwen3-4B
  on a real dropped window (11 → 4 messages, clean summary in the marker).
  Suite **1781 green** (was 1745). Next up: R2-6 (autotune).
