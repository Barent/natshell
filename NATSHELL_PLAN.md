...
| R2-4 | `execute_shell`: **stream** stdout to the TUI as it arrives + **background** handle/`tail`. | R2§6 | ⏳ IN PROGRESS | **Rolled back to a clean boundary 2026-09-10** — first draft of `stream_execute_shell` (asyncio subprocess + chunk pump, callback hook, exit-124/127 parity with `execute_shell`) was written and found to be a security-sensitive re-architecture (sudo stdin, timeout, scrub all re-plumbed) with no consumer in the TUI yet (no event channel for per-chunk rendering; `app.py` only renders on the single `TOOL_RESULT`). **Next-tick design notes:** (1) add `EventType.TOOL_OUTPUT` (chunk text + tool_call id) to `agent/events.py` and a `stream_shell=True` knob on `ToolResult`/registry so only the loop decides which tools stream; (2) `CommandBlock` (ui/widgets.py) needs a `set_partial(text)` + `set_final` pair — the loop's EXECUTING handler mounts the block keyed `cmd-<id>` already, so routing is a one-line `self.query_one(f"#{block_id}")` in `_render_event`; (3) `stream_execute_shell` itself: `asyncio.create_subprocess_shell`, 4 KiB chunk reads, stderr background drain, `asyncio.timeout` → kill + exit-124, `start_new_session=True`, same env filter + LC_ALL + sudo -S / `y\n` rules as the blocking path (reuse the `_inject_sudo_dash_s` / `_PKG_MANAGER_RE` helpers — do NOT duplicate them); Windows falls back to the blocking path (asyncio subprocess unavailable); (4) tests: chunk ordering, terminal byte-parity with `execute_shell` (truncation + sudo scrub), timeout 124 shape, sudo stdin, missing-shell 127 — 8–10 tests pinning each shape; (5) background-handle/`tail` half is a separate sub-step gated on the handle type (needs a `ShellHandle` + `list_shell_handles` tool or `--tail` flag; decide API before building). |
