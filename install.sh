...
cat > "$SYMLINK" << 'WRAPPER'
#!/bin/bash
# NatShell launcher — generated by install.sh
# If this file was overwritten by pip, re-run install.sh to restore GPU support.
NATSHELL_BIN="$HOME/.local/share/natshell/app/.venv/bin/natshell"
if [[ ! -x "$NATSHELL_BIN" ]]; then
    echo "ERROR: NatShell venv not found at $NATSHELL_BIN"
