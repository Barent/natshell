"""Plan generation & per-step prompt templates.

Extracted from :mod:`natshell.agent.plan_executor` (R1-8) so the
model-facing instruction text — the part the prompt-generation tests
pin and the part a prompt reviewer wants to read in one place — is
separate from the pure helpers (step-budget scaling, plan
validation) that stay in ``plan_executor``.

Rendered strings are byte-identical to the pre-extraction version:
``tests/test_plan_execution.py`` covers the small/large context
tiers, the verify-fix budget and the output-truncation behaviour.
"""
...
# Fixed tool budget for the post-verification fix retry (re-exported
# by natshell.agent.plan_executor for the historical import paths).
VERIFY_FIX_BUDGET = 8
